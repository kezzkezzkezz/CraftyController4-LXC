...
pct create "$CTID" "${TEMPLATE_STORAGE}:vztmpl/${TEMPLATE}" -rootfs "local-lvm:vm-${CTID}-disk-0" -cores 4 -memory 2048 -net0 bridge=vmbr0,name=eth0,ip=dhcp --unprivileged 1 >/dev/null
if [ $? -ne 0 ]; then
